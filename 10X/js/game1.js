...
//初始化
function start(){
    var readBoard = storage.getItem("board1");
    if(readBoard == "" || readBoard == undefined || readBoard == null){
        newgame();
    }else{
        //还原单元格
        board = JSON.parse(storage.getItem("board1"));
        for(var i=0;i<5;i++){
            for(var j=0;j<5;j++){
                var cell = $("#cell-"+i+"-"+j);
        ...
        //还原分数
        score = parseInt(storage.getItem("score1"));
        updateScore(score,0);
        //flag为0不显示，为1显示over，为2显示win
        var flag = storage.getItem("flag1");
        switch(flag){
            case "1":
            $('#over').show();
            break;
    ...
    best = Math.max(best,storage.getItem("best1"));
    $('#best').text(best);
}
...
function newgame(){   
    ...
    //重置结束或获胜
    storage.setItem("flag1",0);
    //重置分数
    updateScore(0,0);
    score = 0;
    //缓存分数
    storage.setItem("score1",score);
    saveBoard();
}
...
function clearMultipe()
{
    ...
    bestScore(score);
    //判断获胜条件
    if(score >= 10000 && storage.getItem("flag1") == 0){
        gamewin();
    }
}
...
function gameover(){
    $('#over').fadeIn(2000);
    storage.setItem("flag1",1);
}

//游戏获胜
function gamewin(){
    $('#win').fadeIn(2000);
    storage.setItem("flag1",2);
}

//继续游戏
function continueGame(){
    $('#win').fadeOut();
    storage.setItem("flag1",3);
}
...
function bestScore(score){
    best = Math.max(score,storage.getItem("best1"));
    storage.setItem("best1",best);
    setTimeout(function(){
        $('#best').text(storage.getItem("best1"));
    },1000);
}
...
function saveBoard(){
    ...
    storage.setItem("board1",boardJson);
}