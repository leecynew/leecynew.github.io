...
//初始化
function start(){
    var readBoard = storage.getItem("board2");
    if(readBoard == "" || readBoard == undefined || readBoard == null){
        newgame();
    }else{
        //还原单元格
        board = JSON.parse(storage.getItem("board2"));
        for(var i=0;i<5;i++){
            for(var j=0;j<5;j++){
                var cell = $("#cell-"+i+"-"+j);
        ...
        //还原分数和步数
        score = parseInt(storage.getItem("score2"));
        step = parseInt(storage.getItem("step2"));
        updateScore(score,0);
        //flag为0不显示，为1显示over，为2显示win
        var flag = storage.getItem("flag2");
        switch(flag){
            case "1":
            $('#over').show();
            break;
...
function newgame(){   
    ...
    //重置结束或获胜
    storage.setItem("flag2",0);
    //重置分数和步数
    step = 0;
    updateScore(0,0);
    score = 0;
    //缓存分数和步数
    storage.setItem("score2",score);
    storage.setItem("step2",step);
    saveBoard();
}
...
function saveBoard(){
    ...
    storage.setItem("board2",boardJson);
}